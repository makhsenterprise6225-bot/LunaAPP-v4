  ...
  return (
    <div className={`flex flex-col h-screen ${t.bg} transition-all duration-500 font-sans overflow-hidden`}>
      ...
      <main className="flex-1 overflow-hidden relative px-6 pb-32">
        {/* KEEP-ALIVE MUSIC PLAYER */}
        <div className={`h-full pt-4 h-[85%] ${activeTab === 'music' ? 'block' : 'hidden'}`}>
          <iframe 
          ...
          <div className="h-full pt-4 overflow-y-auto space-y-5">
            ...
            <div className={`${t.acc} p-6 rounded-[32px] text-white shadow-lg flex items-center justify-between`}>
              ...
              <div className="flex gap-2">
                 <button onClick={() => {setTimeLeft(25*60); setIsActive(false)}} className="hover:bg-white/20 p-3 rounded-2xl active:scale-90 transition-all">
                  <RotateCcw size={20} />
                </button>
            ...
            <div className="space-y-3 pb-10">
              {tasks.map((tk: Task) => (
                <div key={tk.id} onClick={() => toggleTask(tk.id)} className={`p-5 rounded-3xl flex bg-white shadow-sm border ${t.border} items-center justify-between group cursor-pointer active:scale-95 transition-all`}>
                  ...
                  <button onClick={(e) => {e.stopPropagation(); setTasks(tasks.filter((x: Task)=>x.id!==tk.id))}} className="p-1 text-slate-300 hover:text-red-400 active:scale-75 transition-all">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
          ...
          <div className="h-full pt-4 space-y-4 flex flex-col overflow-y-auto">
              ...
              <div className="flex justify-between mb-4">
                {['😊', '🤩', '😐', '😔', '😴'].map(m => (
                  <button 
                    ...
                    className={`text-2xl p-2 rounded-xl transition-all ${selectedMood === m ? 'bg-white shadow-md scale-110' : 'opacity-40 active:scale-125'}`}
                  >
                    {m}
                  </button>
                ))}
            ...
            <div className="space-y-4 pb-10">
              {savedEntries.map((entry: JournalEntry) => (
                <div key={entry.id} className="bg-white p-5 rounded-[32px] border border-slate-50 shadow-sm transition-all active:scale-[0.98]">
        ...
        {activeTab === 'profile' && (
          <div className="h-full pt-4 overflow-y-auto pb-10 space-y-6">
              ...
              <div className="flex justify-center gap-6">
                {(['rose', 'midnight', 'forest'] as ThemeKey[]).map(n => (
                  <button 
                    key={n} 
                    onClick={() => setTheme(n)} 
                    className={`w-12 h-12 rounded-full border-4 ${theme===n ? 'border-slate-800 scale-110 shadow-lg':'border-white'} ${n==='rose'?'bg-[#FF8E8E]':n==='midnight'?'bg-[#818CF8]':'bg-[#4ade80]'} active:scale-125 transition-all`} 
                  />
                ))}
              </div>
            </div>
            <div className="grid gap-3">
              ...
              {reasons.map((r, i) => (
                <div key={i} className={`bg-white/50 p-4 rounded-2xl flex items-center gap-3 border ${t.border} transition-all active:translate-x-2`}>
      ...
      <nav className="fixed bottom-8 left-6 right-6 bg-white/90 backdrop-blur-2xl p-3 flex justify-around rounded-[32px] shadow-2xl border border-white">
        {[{id:'music', i:<Music/>}, {id:'tasks', i:<CheckCircle2/>}, {id:'journal', i:<PenTool/>}, {id:'profile', i:<Heart/>}].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`p-4 rounded-2xl transition-all active:scale-75 ${activeTab===tab.id ? `${t.acc} text-white shadow-lg` : `${t.nav}`}`}>{tab.i}</button>
        ))}
      </nav>
    </div>
  );
