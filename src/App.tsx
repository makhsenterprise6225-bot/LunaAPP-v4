...
// Define strictly typed interfaces for production
interface Task {
  id: number;
  text: string;
  completed: boolean;
            ...
            <div className="space-y-3 pb-10">
              {tasks.map((tk: Task) => (
                <div key={tk.id} onClick={() => toggleTask(tk.id)} className={`p-5 rounded-3xl flex bg-white shadow-sm border ${t.border} items-center justify-between group cursor-pointer transition-all hover:translate-x-1`}>
                  ...
                  <button onClick={(e) => {e.stopPropagation(); setTasks(tasks.filter((x: Task)=>x.id!==tk.id))}} className="opacity-0 group-hover:opacity-100 p-1 text-slate-300 hover:text-red-400 transition-all">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
